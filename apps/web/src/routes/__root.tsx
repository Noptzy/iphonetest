		...
		<div className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f] font-sans flex flex-col">
			<header className="sticky top-0 z-50 bg-[#f5f5f7]/80 backdrop-blur-lg border-b border-slate-200/50">
				<nav className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-3">
					<Link to="/" className="text-xl font-semibold tracking-tight hover:text-black transition-colors">
						iPhone Marketplace
					</Link>
					<div className="flex items-center gap-6 text-sm font-medium text-slate-500">
						...
						{user ? (
							<>
								...
								<button type="button" onClick={signOut} className="hover:text-black transition-colors">
									Sign out
								</button>
							</>
						) : (
							<Link to="/login" className="rounded-full bg-black px-4 py-1.5 text-white hover:bg-slate-800 transition-colors">
								Sign in
							</Link>
						)}
					</div>
